// controller/book/about/authorProfileController.js
...
async function fetchWikidataEntity(wikiId) {
  ...
  if (!response.ok) throw new Error(`Failed to fetch entity ${wikiId}`);
  const data = await response.json();
  return data?.entities?.[wikiId] || null;
}
...
async function resolveLabels(qids, lang = "en") {
  ...
  for (let i = 0; i < ids.length; i += 50) {
    ...
    const response = await fetch(url);
    if (!response.ok) throw new Error(`Failed to resolve labels batch`);
    const data = await response.json();
...
async function getAuthorFullProfile(req, res) {
  try {
    ...
    const itemPidList = [
      ...
      "P27","P140","P69","P108","P463","P166","P737",
    ...
    // ✅ profession names + QIDs
    const professionQids = getItemIds(entity, "P106");
    const profession = professionQids.map((id) => labelMap[id]);
