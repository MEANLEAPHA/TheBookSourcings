...
async function getAuthorFullProfile(req, res) {
  try {
    ...
    const literaryMovement = getItemIds(entity, "P135").map((id) => labelMap[id]);
    const languagesOfWork = getItemIds(entity, "P1412").map((id) => labelMap[id]);
    ...
    const employer = getItemIds(entity, "P108").map((id) => labelMap[id]);
    const memberOf = getItemIds(entity, "P463").map((id) => labelMap[id]);

    const notableWorks = getItemIds(entity, "P800").map((id) => labelMap[id]);
    const bibliography = getItemIds(entity, "P527").map((id) => labelMap[id]);
    const awards = getItemIds(entity, "P166").map((id) => labelMap[id]);

    const influencedBy = getItemIds(entity, "P737").map((id) => labelMap[id]);
    const influenced = []; // reverse query needed

    const students = getItemIds(entity, "P802").map((id) => labelMap[id]);
    const teachers = getItemIds(entity, "P1066").map((id) => labelMap[id]);
    const spouse = getItemIds(entity, "P26").map((id) => labelMap[id]);
    const children = getItemIds(entity, "P40").map((id) => labelMap[id]);
    const relatives = getItemIds(entity, "P1038").map((id) => labelMap[id]);
