...
async function buildPersonalizedTrendingFeed(seed, memberQid) {
  try {
    ...
    const topGenres = await getTopGenresForUser(memberQid, 2);
...
async function getPersonalizedAuthors(memberQid, limit = 3) {
  if (!memberQid) {
    return [];
  }
...
async function buildExtendedFeed({ memberQid, cursor, limit }) {
  ...
  const [topGenres, topAuthors] = await Promise.all([
    getPersonalizedGenres(memberQid, 3),
    getPersonalizedAuthors(memberQid, 3)
  ]);
