...
const noNotiIcon = document.querySelector(".fa-otter");
const noNotiSpan = document.querySelector(".no-noti-span");
...
async function loadFollowNotifications() {
  try {
    ...
    if (!data.notifications || data.notifications.length === 0) {
      ...
      // Show placeholder
      if (noNotiIcon) noNotiIcon.style.display = "block";
      if (noNotiSpan) noNotiSpan.style.display = "block";
    ...
    if (noNotiIcon) noNotiIcon.style.display = "none";
    if (noNotiSpan) noNotiSpan.style.display = "none";
