...
async function fetchBookInfo(qid) {
  ...
  const cover =
    getStrings(entity, "P5775")[0] || getStrings(entity, "P18")[0] || "";

  // ✅ Gutenberg first, fallback to OpenLibrary
...
async function getAuthorFullProfile(req, res) {
  try {
    ...
    // ✅ notable works with QID + name + cover + idValue
const notableWorks = [];
for (const id of getItemIds(entity, "P800")) {
  const { cover, idValue } = await fetchBookInfo(id);
  notableWorks.push({
    qid: id,
    name: labelMap[id],
    cover,
    idValue,   // ✅ Gutenberg first, else OpenLibrary
  });
}
